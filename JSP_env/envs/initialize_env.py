...
def define_production_parameters(env, episode):
    ...
    _extend_agent_parameters(parameters=parameters)
    _extend_production_parameters(parameters=parameters)

    # Export parameter config to csv pd.DataFrame.from_dict(parameters, orient="index").to_csv(parameters[
...
def _extend_agent_parameters(parameters):
    ...
    # parameters.update({'STATE_SPACE_PADDING': False})
    # parameters.update({'ACTION_SPACE_PADDING': False})

def _extend_production_parameters(parameters):
